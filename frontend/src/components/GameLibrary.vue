...
function coverUrl(version) {
  const art = version.artwork?.find(a => a.artworkType.toLowerCase() === 'cover')
    ?? version.artwork?.[0]
  if (!art) return null
  return `/mediaitems/VideoGameVersion/${enc(version._itemTitle)}/.artwork/${enc(art.fileName)}`
}

function bannerUrl(version) {
  const art = version.artwork?.find(a => a.artworkType.toLowerCase() === 'banner')
  if (!art) return null
  return `/mediaitems/VideoGameVersion/${enc(version._itemTitle)}/.artwork/${enc(art.fileName)}`
}
</script>
